...
import grpc
from thorchain_proto.proto.cosmos.auth.v1beta1 import auth_pb2 as auth_type
from thorchain_proto.proto.cosmos.auth.v1beta1 import query_pb2 as auth_query
from thorchain_proto.proto.cosmos.auth.v1beta1 import query_pb2_grpc as auth_query_grpc
from thorchain_proto.proto.cosmos.authz.v1beta1 import query_pb2 as authz_query
from thorchain_proto.proto.cosmos.authz.v1beta1 import query_pb2_grpc as authz_query_grpc
from thorchain_proto.proto.cosmos.bank.v1beta1 import query_pb2 as bank_query
from thorchain_proto.proto.cosmos.bank.v1beta1 import query_pb2_grpc as bank_query_grpc
from thorchain_proto.proto.cosmos.base.abci.v1beta1 import abci_pb2 as abci_type
from thorchain_proto.proto.cosmos.base.tendermint.v1beta1 import (
    query_pb2 as tendermint_query,
)
from thorchain_proto.proto.cosmos.base.tendermint.v1beta1 import (
    query_pb2_grpc as tendermint_query_grpc,
)
from thorchain_proto.proto.cosmos.tx.v1beta1 import service_pb2 as tx_service
from thorchain_proto.proto.cosmos.tx.v1beta1 import service_pb2_grpc as tx_service_grpc
from packaging import version

from thorchainpy import pytypes
...
class GrpcClient:
    ...
    @staticmethod
    def assert_compatible_versions(thorchain_proto_version, chain_thor_version):
        """
        ...
        """
        if thorchain_proto_version[0] == "v":
            thorchain_proto_version = thorchain_proto_version[1:]
        if chain_thor_version[0] == "v":
            chain_thor_version = chain_thor_version[1:]

        if len(chain_thor_version) >= GITHUB_COMMIT_HASH_LEN:
            logger = logging.getLogger("client-logger")
            logger.warning(
                f"The chain is running a custom release from branch/commit {chain_thor_version}. "
            ...
            error_string = (
                f"Version error, Python sdk runs with thorchain protobuf version {thorchain_proto_version}, but the "
                f"remote chain is running with version {chain_thor_version}"
            )

            assert (
                version.parse(thorchain_proto_version).major
                >= version.parse(chain_thor_version).major
            ) and (
                version.parse(thorchain_proto_version).minor
                >= version.parse(chain_thor_version).minor
            ), error_string
